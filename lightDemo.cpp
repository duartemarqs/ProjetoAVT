...
void processKeys(unsigned char key, int xx, int yy)
{
	...
	case 'g':
		if (directionalLightState) flareEffect = false;
		else
...
int main(int argc, char** argv) {
	...
	Light directionalLight = Light(positions[6], colors[6], states[6]);
	float lDir[3] = { 2.0f, 0.0f, 0.0f };
	directionalLight.setDirection(lDir);
	lights.push_back(directionalLight); // Directional light
